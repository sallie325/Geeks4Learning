...
import {
  ...
  maxPauseCount,
  maxToastrTimeout,
} from '../shared/constants/goal-boundaries';
import {
  archivedState,
  backlogState,
  completedState,
...
import {
  getCompletedTasks,
  getPastTenseFromGoalState,
  getPresentTenseFromGoalState,
  getRandomCustomPauseMessage,
  getRandomCustomSuccessMessage,
} from './services/helpers/goal-service.helpers';
...
export class GoalManagementComponent implements OnInit, OnDestroy {
      ...
      if (previousContainerLinks.includes(event.container.id)) {
        ...
        switch (event.container.id) {
              ...
              this.goalCommentService
                ...
                .subscribe((userComment: string | null) => {
                  ...
                  if (userComment) {
                    ...
                    event.previousContainer.data[
                      event.previousIndex
                    ]?.tasks?.forEach((element: GoalTaskModel) => {
          ...
          case startedState:
            // Guard condition
            if (
              !this.goalValidationsService.canGoalBeStarted(
          ...
          case completedState:
            if (
              ...
              if (
                !this.goalValidationsService.canCompleteGoal(
                  getCompletedTasks(
                    event.previousContainer.data[event.previousIndex]?.tasks
                  ),
                  event.previousContainer.data[event.previousIndex]?.tasks
        ...
        this.toastrMessageService.showErrorMessage(
          'Change Goal State',
          `Invalid goal operation! You cannot ${getPresentTenseFromGoalState(
            event.container.id
          )} a goal that is ${getPastTenseFromGoalState(
  ...
  updateGoalChanges(event: CdkDragDrop<any[], any[], any>) {
    ...
    this.goalService
      .updateGoal(event.previousContainer.data[event.previousIndex])
